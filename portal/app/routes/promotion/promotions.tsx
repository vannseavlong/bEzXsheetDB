...
import { promotion } from '@/constants/data-dummy';
...
import { PromotionColumns } from '@/components/data-table/promotion-columns';
import PromotionHeader from '@/components/data-table/promotion-header';

export default function ProductOption() {
  const tableState = useTableState();
  const table = useDataTableConfig(promotion, PromotionColumns, tableState);
  const {
    statusFilter,
    dateRange,
    isCalendarOpen,
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-88px)] overflow-hidden p-4 pb-0">
      <div className="rounded-md border flex flex-col flex-1 min-h-0">
        <PromotionHeader {...headerProps} />
        <div className="flex min-h-0 overflow-hidden">
          <Table className="min-w-full">
            <TableHeader table={table} />
            <TableBody>
              <TableRows columns={PromotionColumns} table={table} />
            </TableBody>
          </Table>
        </div>
      </div>
